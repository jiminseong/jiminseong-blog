...
export function Profile({ name, profileImage, githubUrl, linkedinUrl }: Props) {
  return (
    <div className="flex items-center gap-2.5">
      <div className="rounded-full w-7 h-7 relative overflow-hidden ring-1 ring-slate-300/60 dark:ring-slate-700/70 shrink-0">
        <Image fill alt={name} src={profileImage} className="object-cover" sizes="28px" />
      </div>
      <div className="flex items-center gap-2 text-slate-500 dark:text-slate-300">
        <Link
          ...
          aria-label="GitHub"
          className="flex h-7 w-7 items-center justify-center hover:text-slate-900 dark:hover:text-white transition-colors"
        >
