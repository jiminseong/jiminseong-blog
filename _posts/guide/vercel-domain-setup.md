...
1. **가비아**에서 도메인을 산다.
2. **Vercel**에 그 도메인을 추가하고, 적을 IP 주소를 확인한다.
...
> **직접 클릭하기 귀찮으신가요?**
> 화면을 보고 대신 클릭해 주는 AI에게 아래 문장을 그대로 주면, 이 글의 과정을 통째로 맡길 수 있어요. Aside 브라우저, ChatGPT 에이전트 모드, Claude의 Computer Use 어디에 넣어도 됩니다. 대괄호 안 두 가지만 여러분 것으로 바꾸세요.
...
> 결제나 저장처럼 되돌리기 어려운 순간에는 멈추고 사람에게 넘기도록 규칙을 넣어 두었어요. 이런 도구가 처음이라면 [Aside 브라우저 가이드](/guide/aside-browser-setup)와 [GPT-6 Astra 설명글](/guide/gpt-6-astra)을 먼저 읽어 보세요. 아래부터는 직접 손으로 하는 방법이에요.
...
# 2부. Vercel에 도메인 추가하고 IP 주소 확인하기

이제 Vercel로 갑니다. 여기서 확인한 IP 주소를 3부에서 가비아에 적을 거예요.
...
1. **가비아**에서 도메인을 샀어요.
2. **Vercel**에 도메인을 추가하고 IP 주소를 확인했어요.
3. **가비아** DNS 관리에 그 IP 주소를 적었어요.
